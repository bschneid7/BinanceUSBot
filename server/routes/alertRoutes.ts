...
router.get('/', requireUser(), async (req: AuthRequest, res: Response) => {
  try {
    ...
    if (level && ['INFO', 'WARNING', 'ERROR', 'CRITICAL'].includes((level as string).toUpperCase())) {
      alerts = await alertService.getAlertsByLevel(
        userId,
        (level as string).toUpperCase() as 'INFO' | 'WARNING' | 'ERROR' | 'CRITICAL',
        alertLimit
      );
    }
    // Filter by type if specified
    else if (type) {
      alerts = await alertService.getAlertsByType(userId, type as string, alertLimit);
    }
