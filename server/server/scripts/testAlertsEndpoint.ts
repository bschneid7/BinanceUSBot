...
async function testAlertsEndpoint() {
  ...
  try {
    ...
    console.log('  Alerts:');
    alertsResponse2.data.alerts.forEach((alert: any, index: number) => {
    ...
    alertsResponse3.data.alerts.forEach((alert: any, index: number) => {
    ...
    alertsResponse4.data.alerts.forEach((alert: any, index: number) => {
      console.log(`    ${index + 1}. [${alert.level}] ${alert.message}`);
    });
    console.log();
