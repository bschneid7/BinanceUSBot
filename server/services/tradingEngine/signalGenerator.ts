...
export interface TradingSignal {
  ...
  reason: string;
  isEvent?: boolean;
  tier?: string; // Signal tier (TIER_1_CONSERVATIVE, TIER_2_MODERATE, TIER_3_AGGRESSIVE)
...
export class SignalGenerator {
  ...
  private async checkPlaybookA(
    ...
    try {
      ...
      // Use Tier 2 (moderate) volume threshold: 1.5x instead of 1.8x
      const tier2VolumeThreshold = SIGNAL_TIERS.TIER_2_MODERATE.playbook_multipliers.volume_mult;
      if (volumeMultiple < tier2VolumeThreshold) {
        console.log(`[PlaybookA] ${symbol} - Volume ${volumeMultiple.toFixed(2)}x < ${tier2VolumeThreshold}x required (Tier 2)`);
        return null;
      }
