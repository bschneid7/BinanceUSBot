...
export class ExecutionRouter {
  ...
  async executeSignal(
    ...
    try {
      ...
      // Phase 3: Correlation-Based Risk Management (only for new positions)
      if (!positionId) {
        const positionManager = (await import('./positionManager')).default;
        const correlationCheck = await positionManager.checkCorrelationRisk(userId, signal.symbol);
        
        if (!correlationCheck.canOpen) {
          logger.warn(
            `[ExecutionRouter] Correlation risk check failed: ${correlationCheck.reason} - Correlated with: ${correlationCheck.correlatedSymbols?.join(', ')}`
          );
          return {
            success: false,
            error: `Correlation risk: ${correlationCheck.reason}`,
          };
        }
      }
