 ...
 */
export const updateUserConfig = async (userId, updates) => {
    try {
        ...
        const config = await BotConfig.findOneAndUpdate({ userId }, { $set: updates }, { new: true, runValidators: true });
        if (!config) {
 ...
 */
export const deleteUserConfig = async (userId) => {
    try {
