...
async function testConfigEndpoints() {
    ...
    try {
        ...
        console.log('\n=== Step 3: Update Risk Settings ===');
        try {
            const updateResponse = await axios.put(`${API_BASE_URL}/api/config`, {
                risk: {
                    R_pct: 0.008,
                    max_positions: 5,
        ...
        try {
            const getResponse = await axios.get(`${API_BASE_URL}/api/config`, { headers });
            const updatedConfig = getResponse.data.config;
            const checks = [
        ...
        console.log('\n=== Step 8: Test Validation (Invalid R_pct) ===');
        try {
            await axios.put(`${API_BASE_URL}/api/config`, {
                risk: {
...
// Run tests
testConfigEndpoints();
//# sourceMappingURL=testConfigEndpoint.js.map