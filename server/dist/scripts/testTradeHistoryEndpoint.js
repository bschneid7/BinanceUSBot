...
class TradeHistoryEndpointTester {
     ...
     * Add test result
     */
    addResult(testName, passed, message, data) {
    ...
    async testGetAllTradeHistory() {
        try {
            ...
            if (response.status === 200 && Array.isArray(response.data.trades)) {
                this.addResult('Get All Trade History', true, `Retrieved ${response.data.trades.length} trades`, { count: response.data.trades.length });
                // Verify structure of first trade
                if (response.data.trades.length > 0) {
                    const firstTrade = response.data.trades[0];
                    const hasRequiredFields = 'symbol' in firstTrade &&
                        'playbook' in firstTrade &&
                        'outcome' in firstTrade &&
                        'pnl_usd' in firstTrade &&
                        'date' in firstTrade;
                    this.addResult('Trade Structure Validation', hasRequiredFields, hasRequiredFields ? 'Trade object has all required fields' : 'Missing required fields', { sampleTrade: firstTrade });
                }
            }
            else {
        ...
        try {
            ...
            for (const playbook of playbooks) {
                ...
                if (response.status === 200) {
                    ...
                    const allMatchPlaybook = trades.every((trade) => trade.playbook === playbook);
                    this.addResult(`Filter by Playbook ${playbook}`, allMatchPlaybook, allMatchPlaybook
                        ? `Found ${trades.length} trades for playbook ${playbook}`
                        : `Some trades don't match playbook ${playbook}`, { count: trades.length, playbook });
                }
            }
        }
        catch (error) {
    ...
    async testFilterByOutcome() {
        try {
            ...
            for (const outcome of outcomes) {
                ...
                if (response.status === 200) {
                    const trades = response.data.trades;
                    const allMatchOutcome = trades.every((trade) => trade.outcome === outcome);
                    this.addResult(`Filter by Outcome ${outcome}`, allMatchOutcome, allMatchOutcome
                        ? `Found ${trades.length} trades with outcome ${outcome}`
                        : `Some trades don't match outcome ${outcome}`, { count: trades.length, outcome });
                }
            }
        }
        catch (error) {
    ...
    async testFilterBySymbol() {
        try {
            ...
            for (const symbol of symbols) {
                ...
                if (response.status === 200) {
                    const trades = response.data.trades;
                    const allMatchSymbol = trades.every((trade) => trade.symbol === symbol);
                    this.addResult(`Filter by Symbol ${symbol}`, allMatchSymbol, allMatchSymbol
                        ? `Found ${trades.length} trades for ${symbol}`
                        : `Some trades don't match symbol ${symbol}`, { count: trades.length, symbol });
                }
            }
        }
        catch (error) {
    ...
    async testFilterByDateRange() {
        try {
            ...
            if (response1.status === 200) {
                ...
                const allAfterDate = trades.every((trade) => new Date(trade.date) >= new Date(oneDayAgo));
                this.addResult('Filter by Date (Last 24h)', allAfterDate, allAfterDate
                    ? `Found ${trades.length} trades in last 24 hours`
                    : 'Some trades are outside date range', { count: trades.length, startDate: oneDayAgo });
            }
            // Test 2: Last 7 days
            ...
            if (response2.status === 200) {
                ...
                const allAfterDate = trades.every((trade) => new Date(trade.date) >= new Date(sevenDaysAgo));
                this.addResult('Filter by Date (Last 7 days)', allAfterDate, allAfterDate
                    ? `Found ${trades.length} trades in last 7 days`
                    : 'Some trades are outside date range', { count: trades.length, startDate: sevenDaysAgo });
            }
            ...
            if (response3.status === 200) {
                ...
                const allInRange = trades.every((trade) => {
                    const tradeDate = new Date(trade.date);
                    return tradeDate >= new Date(sevenDaysAgo) && tradeDate <= new Date(threeDaysAgo);
                });
                this.addResult('Filter by Date Range (7d to 3d ago)', allInRange, allInRange
                    ? `Found ${trades.length} trades in date range`
                    : 'Some trades are outside date range', { count: trades.length, startDate: sevenDaysAgo, endDate: threeDaysAgo });
            }
        }
        catch (error) {
    ...
    async testCombinedFilters() {
        try {
            ...
            if (response1.status === 200) {
                const trades = response1.data.trades;
                const allMatch = trades.every((trade) => trade.playbook === 'A' && trade.outcome === 'WIN');
                this.addResult('Combined Filter (Playbook A + WIN)', allMatch, allMatch
                    ? `Found ${trades.length} winning trades for playbook A`
                    : 'Some trades don\'t match combined filters', { count: trades.length });
            }
            ...
            if (response2.status === 200) {
                const trades = response2.data.trades;
                const allMatch = trades.every((trade) => trade.symbol === 'BTCUSDT' && new Date(trade.date) >= new Date(sevenDaysAgo));
                this.addResult('Combined Filter (BTCUSDT + Last 7d)', allMatch, allMatch
            ...
            if (response3.status === 200) {
                const trades = response3.data.trades;
                const allMatch = trades.every((trade) => trade.symbol === 'ETHUSDT' &&
                    trade.playbook === 'B' &&
                    trade.outcome === 'WIN' &&
                    new Date(trade.date) >= new Date(sevenDaysAgo));
                this.addResult('Combined Filter (All filters)', allMatch, allMatch
                    ? `Found ${trades.length} trades matching all filters`
                    : 'Some trades don\'t match all filters', { count: trades.length, filters: { symbol: 'ETHUSDT', playbook: 'B', outcome: 'WIN', startDate: sevenDaysAgo } });
            }
        }
        catch (error) {
    ...
    async testEdgeCases() {
        try {
            ...
            if (response1.status === 200) {
                const trades = response1.data.trades;
                this.addResult('Edge Case (Invalid Playbook)', trades.length === 0, trades.length === 0
                    ? 'Correctly returned empty array for invalid playbook'
                    : `Unexpectedly found ${trades.length} trades`, { count: trades.length });
            }
            ...
            if (response2.status === 200) {
                const trades = response2.data.trades;
                this.addResult('Edge Case (Non-existent Symbol)', trades.length === 0, trades.length === 0
            ...
            if (response3.status === 200) {
                const trades = response3.data.trades;
                this.addResult('Edge Case (Future Date)', trades.length === 0, trades.length === 0
...
//# sourceMappingURL=testTradeHistoryEndpoint.js.map